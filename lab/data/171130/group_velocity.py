...
root = 'lab/data/171130/plate2/200v10p150mm2cm40mmX'  # 修改路径
for i in ['13.csv', '14.csv', '15.csv', '16.csv']:
...
for (t, r, s, a) in zip(time, ref, sig, angle):
    for (pt, pr, ps) in zip(t, r, s):
        ...
        if (ps > 30) and (t_end == 0):
            t_end = pt
    if (t_end > t_start):
        distance = (150e-3+30e-3*2)-(2*45e-3*math.tan(a*np.pi/180)) # 单位m 修改45
